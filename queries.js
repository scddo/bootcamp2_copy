...
async function retrieveAllListings() {
  try {
    ...
    console.log('Retrieving all listings:', listings);
  } 
  
  catch (error) {
    console.error('Unable to retrieve listings:', error);
  }
}
...
async function removeCable() {
  // ADD CODE HERE
  const listing = await Listing.destroy({ where: { code: 'CABL' }});
...
async function addDSIT() {
  ...
  try {
    ...
    console.log('New listing added:', newListing.toJSON());
  } 
...
async function updatePhelpsLab() {
  // ADD CODE HERE
  try {
    const phelpsLab = await Listing.findOne({ where: { name: 'Phelps Laboratory' } });
    console.log('Finding Phelps Laboratory:', phelpsLab);
    await phelpsLab.update({ address: '1275 Center Drive, Gainesville, FL 32611, United States' });
    console.log('Updated Phelps Laboratory:', phelpsLab);
  } 
  
  catch (error) {
    console.error('Unable to update Phelps Laboratory:', error);
  }
}
